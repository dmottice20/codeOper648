...
f1 = open("data/problem4_partc_u_star.pkl", "wb")
pickle.dump(u_star, f1)
f1.close()
# Save the hash of reward vectors...
f2 = open("data/problem4_partc_d_star.pkl", "wb")
...
for s in S:
    P_pi_star[s, :] = P[d_star_int[s][0]][s, :]

with open("data/induced_p_matrix_problem3.npy", "wb") as f:
    np.save(f, P_pi_star)

P = P_pi_star
logging.info(f"The induced P matrix is...\n{P}")
A_top_row = np.identity(P.shape[0])-P.transpose()
A_bottom_row = np.repeat(1, P.shape[0])
A = np.vstack((A_top_row, A_bottom_row))
A = np.delete(A, 0, axis=0)
b = np.vstack((np.asarray([np.repeat(0, P.shape[0]-1)]).transpose(), [1]))
pi = np.matmul(np.linalg.inv(A), b)
logging.info(f"The limiting distribution is...\n{pi}")