...
def run_gauss_seidel_modified_policy_iteration(P, r, A, S, epsilon, lamb, m, max_iter):
    """
    ...
    :return: NOT SURE YET
    """
    print("Running Gauss Seidel modified policy iteration...")
    start = time.time()
    card_s = len(S)
    card_a = len(A)
    # Step 1:
    ...
    while n < max_iter:
        ...
        if conv_val >= conv_test:
            while k < m:
                ...
                S = S[::-1]
                for s_i in S:
                    unkp1[s_i-1] = r[int(dnp1[s_i-1])-1][s_i-1] + lamb * P[int(dnp1[s_i-1])-1][s_i-1, :] @ unkp1
